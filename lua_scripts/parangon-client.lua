...
parangon.mainWindow:RegisterForDrag("LeftButton", "RightButton")
parangon.mainWindow:SetMovable(true)
parangon.mainWindow:SetScript("OnDragStart", parangon.mainWindow.StartMoving)
...
parangon.mainWindowTexture:SetTexCoord(0.58154296875, 0.96435546875,
    0.04052734375, 0.81201171875)

parangon.mainTitle = CreateFrame("Frame", parangon.mainTitle, parangon
    .mainWindow)
...
parangon.mainTitleTexture:SetTexCoord(0.23486328125, 0.48779296875, 0.33251953125,
    0.40966796875)

parangon.mainTitleText = parangon.mainTitle:CreateFontString(parangon
    .mainTitleText)
...
parangon.mainWindowArt = CreateFrame("Button", parangon.mainWindowArt,
    parangon.mainWindow)
...
parangon.mainWindowArtTexture:SetTexCoord(0.00048828125, 0.35009765625,
    0.00048828125, 0.21435546875)

parangon.levelWindow = CreateFrame("Frame", parangon.levelWindow,
    parangon.mainWindow)
...
parangon.levelWindowTexture:SetTexCoord(0.05419921875, 0.17431640625,
    0.31201171875, 0.42724609375)
...
parangon.closeButton = CreateFrame("Button", parangon.closeButton,
    parangon.mainWindow, "UIPanelCloseButton")
...
parangon.expIcon:SetPoint("TOP", 0, -120)
...
parangon.expText:SetPoint("TOP", 0, -160)
...
for id, subtable in pairs(parangon.spellsList) do
    parangon.leftButtons[id] = CreateFrame("Frame", parangon.leftButtons[id],
        parangon.mainWindow)
    parangon.leftButtons[id]:SetSize(50, 50)
    parangon.leftButtons[id]:SetPoint("LEFT", 20,
        parangon.buttonsCoords.global.pos_y)
    ...
    parangon.leftButtonsArt[id] = CreateFrame("Frame",
        parangon.leftButtonsArt[id], parangon.leftButtons[id], nil)
    ...
    parangon.centerButtons[id] = CreateFrame("Button", parangon.centerButtons
        [id], parangon.mainWindow, nil)
    parangon.centerButtons[id]:SetSize(170, 55)
    parangon.centerButtons[id]:SetPoint("CENTER", 0,
        parangon.buttonsCoords.global.pos_y)
    parangon.centerButtons[id]:SetNormalTexture(
        "Interface/Parangon/LargeButtonBorder")
    parangon.centerButtons[id]:SetHighlightTexture(
        "Interface/Parangon/LargeButtonBorder_Hover")
    parangon.centerButtons[id]:SetPushedTexture(
        "Interface/Parangon/LargeButtonBorder_Push")
    parangon.centerButtons[id]:EnableMouseWheel(1)
    parangon.centerButtons[id]:SetFrameLevel(3)

    parangon.centerButtons[id]:SetScript("OnMouseUp",
    ...
    parangon.rightButtons[id] = CreateFrame("Frame", parangon.rightButtons[id],
        parangon.mainWindow)
    parangon.rightButtons[id]:SetSize(50, 50)
    parangon.rightButtons[id]:SetPoint("Right", -20,
        parangon.buttonsCoords.global.pos_y)
    parangon.rightButtons[id]:SetFrameLevel(1000)
    parangon.rightButtons[id]:SetFrameLevel(3)

    parangon.rightText[id] = parangon.rightButtons[id]:CreateFontString(parangon
        .rightText[id])
    ...
    parangon.rightButtonsTexture[id]:SetTexture(
        "Interface/parangon/ButtonBorder")

    parangon.buttonsCoords.global.pos_y = parangon.buttonsCoords.global.pos_y -
        60
end
...
parangon.characterFrameContainer = CreateFrame("Frame",
    parangon.characterFrameContainer, CharacterFrame);
...
parangon.characterFrameContainer:SetScript("OnDragStart",
    parangon.characterFrameContainer.StartMoving)
parangon.characterFrameContainer:SetScript("OnHide",
    parangon.characterFrameContainer.StopMovingOrSizing)
parangon.characterFrameContainer:SetScript("OnDragStop",
    parangon.characterFrameContainer.StopMovingOrSizing)

parangon.characterFrameBorder = CreateFrame("Button",
    parangon.characterFrameBorder, parangon.characterFrameContainer)
...
parangon.characterFrameBorder:SetHighlightTexture(
    "Interface/parangon/ButtonBorder_Hover")
parangon.characterFrameBorder:SetPushedTexture(
    "Interface/parangon/ButtonBorder_Push")
...
parangon.characterFrameBorder:SetScript("OnEnter", function(self, button, down)
    ...
    for spellid, subtable in pairs(parangon.spellsList) do
        GameTooltip:AddLine("|CFFFFFFFF+ " ..
            parangon.rightText[spellid]:GetText() .. "|CFFFFFFFF " ..
            subtable.name .. "|r");
    end
...
parangon.characterFrameBackground = CreateFrame("Frame",
    parangon.characterFrameBackground, parangon
    .characterFrameBorder)
